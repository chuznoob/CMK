...
$classContact = new Contacto();
$dataContact=$classContact->show_data(NULL);
...
<head>
<!-- ******************************************HEADER********************************************************* -->
        <title>Admin/Contactos_CM</title>
...
<?php } ?>   
                    ...
                    <li>
                        <a class="active" href="contactos_cm.php"><i class="fa fa-inbox fa-3x"></i>&nbsp; Bandeja de entrada
...
<!-- ***************************************CONTENIDO********************************************************* -->
        ...
        <div id="page-wrapper" >
            <div id="page-inner">
                <div class="row orion">
                    <div class="col-md-9">
                     <h2 id="four">Panel de mensajes de entrada </h2>   
                        <h5>En este apartado usted podrá revisar aclaraciones, solicitudes o la cotización de algún servicio.</h5>
                    </div>
                            <div class="col-md-3"> 
...
while($data=$dataContact->fetchObject())
{
                        echo "<tr class='odd gradeX '>";
                            echo "<td>" . $data->id_contacto . "</td>";
                            echo "<td>" . $data->nom_cliente . "</td>";
                            echo "<td>" . $data->cor_cliente . "</td>";
                            echo "<td>" . $data->asunto_contacto . "</td>";
                            echo "<td>" . $data->text_contacto . "</td>";
                            echo "<td>" . $data->date_contacto . "</td>";
    
     if($data->status_contacto=="No-Leido"){
                            echo "<td class='text-center'><i class='red-e fa fa-eye-slash fa-2x'></i>
                            <span class='lil-c'>(".$data->status_contacto.")</span></td>";
     }else{
    ...
    if($data->status_contacto=="No-Leido"){
                            echo "<td> <form name='F-update' action='../lib/ops_contacto/changeContacto.php' method='post'>
                                 <input type='hidden' name='id' value='".$data->id_contacto."'>
                                 <input type='hidden' name='status_contacto' value='".$data->status_contacto."'>
                           ...
                           echo "<td> <form name='F-update' action='../lib/ops_contacto/changeContacto.php' method='post'>
                            ...
                            echo "<td> <form name='F-delete' class='validator'action='../lib/ops_contacto/deleteContacto.php' method='post'>
                                 <input type='hidden' name='id' value='".$data->id_contacto."'>
...
&nbsp;Ningún mensaje ha sido enviado desde la página de Compumark&nbsp;
<i class="fa fa-exclamation-circle fa-2x "></i>
</div>                                                                   
 <?php } ?>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                          En esta sección usted podrá eliminar y marcar como “visto”  los mensajes recibidos lo cual le permite mantener un control sobe lo que recibe, para ello solo debe seleccionar el botón que corresponda a la acción que usted desea realizar.
                                        </div>
