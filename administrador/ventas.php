<?php
session_start();
include_once '../lib/class/Compras.php';

if(isset($_POST['filtro'])){
$classComp = new Compras();
$idC=$_POST['filtro'];
$dataComp=$classComp->show_data($idC); 
}else{
$classComp = new Compras();
$idC=NULL;
$dataComp=$classComp->show_data(Null);    
}
...
<?php } ?>   
                    ...
                    <li>
                        <a  class="active" href="ventas.php"><i class="fa fa-line-chart fa-3x"></i>&nbsp; Historial de Ventas</a>
                    </li>
				    <li>
<?php
...
<!-- ***************************************CONTENIDO********************************************************* -->
        <div id="page-wrapper" >
            ...
            <div class="row orion">
                <div class="col-md-9 ">
                 <h2 id="one">Panel de Ventas</h2>   
                        <h5>En esta pantalla usted podrá ver ell historial de ventas de la organizacion.</h5> 
                </div>
                ...
                <div class="col-md-3"> 
                 <button class="btn btn-info btn-sm" data-toggle="modal" data-target="#myModal5">
                             ...
                             <form name="f-filtro" action="#"method="POST">
                             <select name="filtro" class="form-control"
                             onchange="this.form.submit();">
                             <option value="NULL">Todos</option>
                             <?php while($dataCl=$dataCli->fetchObject()){ ?>
                             <option value="<?php echo $dataCl->id_Cliente; ?>"
                             <?php if($dataCl->id_Cliente==$idC){ echo "selected";} ?> >
                             <?php echo $dataCl->nom_cliente; ?>
                            </option>
                             <?php } ?>
                            </select>
                            </form>
...
while($data=$dataComp->fetchObject())
{
                        echo "<tr class='odd gradeX'>";
                            echo "<td>" . $data->id_Compras . "</td>";
                            echo "<td>" . $data->nom_cliente . "</td>";
                            echo "<td>" . $data->cor_cliente . "</td>";
                            echo "<td>" . $data->fech_compra . "</td>";
                            echo "<td>$" . $data->mf_compras . "</td>";
                            echo "<td>$" . $data->desc_compra. "</td>";
                            echo "<td> <form name='details' action='det_Ventas.php' method='post'>
                                 <input type='hidden' name='id_Compras' value='".$data->id_Compras."'>
                                 <button type='submit' class='btn btn-info btn-sm'><i class='fa fa-search fa-x'></i></button>
                                 </form>
                                 </td>";
                        echo "</tr>"; ?>
...
En esta sección usted podrá corroborar las ventas realizadas en la organización, con el selector debajo de información usted podrá aislar las ventas de un solo usuario.    
                                        </div>
